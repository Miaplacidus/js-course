describe("myQuery", function () {
  ...
  describe("Show and Hide", function () {
    ...
    it("hides selected items", function(){
      $("#profile").hide();
      var elem = $('#profile').get(0);
      expect(elem.style.display).toEqual("none");
    });

    it("shows hidden items", function(){
      $('#profile').hide();
      $('#profile').show();
      var elem = $('#profile').get(0);
      expect(elem.style.display).toEqual("initial");
    });
  ...
  describe("addClass", function () {
    ...
    it("can add a class to an element", function(){
      var buttons = $('.button');
      buttons.addClass("click_me");
      expect(buttons.get(0).className).toMatch(/first click_me/);
      expect(buttons.get(1).className).toMatch(/second click_me/);
    });
  });
